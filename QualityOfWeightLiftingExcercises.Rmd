...
## open
- work on description of cross validation
- work on "out of sample error"
- one nice figure of random forest
...
So I built different prediction models, used cross validation, calculated the expected out of sample error of each model and predicted how well the exercise is done on a small given data set.
As result I used the model calculated with Random Forest for the prediction. The reason was a the best expected accuracy.
...
setwd("~/Coursera/DataAnalysis/practical_machine_learning/project/data/")
...
# create own training and test data from pmltrain
library(caret)
set.seed(123)
...
NotSparse <- sapply(training, function(x) mean(is.na(x))) <= 0.95
...
As second method I use "rpart2". Rpart2 has a different optimization algorithm.    
...
glm models can only use 2-class outcomes. So it is not possible to use it in this project because we more than two outcomes in the attribute "classes".
...
After building the model I first build a confusion Matrix with the training data. Then with the testing data.

```{r confusion matrix training, cache=TRUE}
library(caret)
# Confusion Matrix on training data
confusionMatrix(predict(rfFit), training$classe)
```

After set I do a prediction on the testing data and built a confusion matrix.
...
On the testing set it is still a very high accuracy of 99.25%.    
...
# Ergebnis für das Quiz (uncommantated)
#predictpmltest
```
